...
export default function WorkProgrammePanel({
  ...
  // Actual progress map
  // { [programme_item_id]: { pct, source, leafWeight, rollupWeightTotal, weightMode, alphaCost } }
  const [actualProgressMap, setActualProgressMap] = useState({});

  // ✅ Saved configuration per programme version (persisted in programme_versions)
  // weightMode maps to DB weight_mode: 'hybrid' | 'boq' | 'duration'
  ...
  // ------------------------------------------------------------
  // Sync weight config from selected version (DB -> UI)
  // ------------------------------------------------------------
  useEffect(() => {
    const syncFromSelected = async () => {
      if (!contractId || !selectedVersionNo) return;

      // Prefer values if programmeService already included these fields
      const wm = selectedVersion?.weight_mode;
      const ac = selectedVersion?.alpha_cost;

      if (wm) {
        setWeightMode(wm);
        setAlphaCost(ac != null ? Number(ac) : 0.7);
        return;
      }
      ...
      try {
        ...
        setWeightMode(data?.weight_mode || 'hybrid');
        setAlphaCost(data?.alpha_cost != null ? Number(data.alpha_cost) : 0.7);
      } catch (e) {
        console.warn('Could not load weight config; using defaults.', e);
        setWeightMode('hybrid');
        setAlphaCost(0.7);
      }
    };
  ...
  // Debounced to avoid spamming DB.
  // ------------------------------------------------------------
  useEffect(() => {
    if (!contractId || !selectedVersionNo) return;
    if (!canEdit) return;
    ...
    const t = setTimeout(async () => {
      try {
        ...
        // Also update local versions state so UI remains consistent
        setVersions((prev) =>
          (prev || []).map((v) =>
  ...
  // ------------------------------------------------------------
  // Load Actual Progress (ALWAYS from AUTO view)
  // Because AUTO view reads programme_versions.weight_mode + alpha_cost.
  // ------------------------------------------------------------
  useEffect(() => {
                ...
                <div className="bg-white border rounded-lg overflow-hidden">
                  <div className="px-4 py-3 bg-gray-50 border-b">
                    ...
                    <div className="text-xs text-gray-600 mt-0.5">
                      Weight Mode & Alpha are saved into programme_versions per version.
                    </div>
                  </div>
                  ...
                  <div className="p-4 space-y-3">
                    ...
                    {/* Weight configuration (saved per version) */}
                    {selectedVersionNo && (
                        ...
                        <div className="flex items-center justify-between gap-3">
                          <div>
                            ...
                            <div className="text-[11px] text-gray-600">
                              This setting affects how parent actual % is calculated (weighted roll-up).
                            </div>
                          </div>
                        ...
                        <div className="mt-3 grid grid-cols-1 md:grid-cols-2 gap-3">
                          <div>
                            ...
                            <select
                              ...
                              title="This saves into programme_versions.weight_mode"
                            >
                          ...
                          <div>
                            ...
                            <div className="mt-1 flex items-center gap-2">
                              ...
                              <div className="w-16 text-right text-sm font-semibold">
                                {Number(alphaCost).toFixed(2)}
                              </div>
                            </div>
                            <div className="text-[11px] text-gray-600 mt-1">
                              HybridWeight = α×CostWeight + (1−α)×k×DurationWeight
                            </div>
                          </div>
                        </div>
