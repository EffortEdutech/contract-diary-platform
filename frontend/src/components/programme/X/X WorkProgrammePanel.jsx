...
export default function WorkProgrammePanel({
  ...
  authority,
  isLocked,
  // Optional: parent can pass this to know when programme changed
  onChanged,
}) {
  ...
  const [actualByProgrammeId, setActualByProgrammeId] = useState({});
  ...
  const loadItems = async (versionNo) => {
    ...
    try {
      ...
      // Sprint 2C: Load actual progress (to-date) from ledger deltas
      let actualMap = {};
      try {
        // 1) Preferred: read from view (fast)
        const { data: actualRows, error: actualErr } = await supabase
        ...
        if (!actualErr) {
          (actualRows || []).forEach((r) => {
            actualMap[r.programme_item_id] = Number(r.actual_percent_to_date || 0);
          });
        } else {
          // 2) Fallback: compute in client if view missing / schema cache not refreshed / policy not ready
          console.warn('[WorkProgrammePanel] actual progress view not ready, using fallback:', actualErr);
          const { data: rows, error: e2 } = await supabase
            .from('diary_programme_links')
            .select('programme_item_id, progress_update, programme_items!inner(contract_id)')
            .eq('programme_items.contract_id', contractId);

          if (e2) {
            console.warn('[WorkProgrammePanel] actual progress fallback failed:', e2);
          } else {
            (rows || []).forEach((r) => {
              const k = r.programme_item_id;
              const v = Number(r.progress_update || 0);
              if (!Number.isFinite(v)) return;
              actualMap[k] = Math.min(100, (actualMap[k] || 0) + v);
            });
          }
        }
      } catch (e) {
        console.warn('[WorkProgrammePanel] actual progress load threw:', e);
      }

      setActualByProgrammeId(actualMap);

      const merged = (list || []).map((it) => ({
        ...it,
        actual_percent_to_date: actualMap[it.id] ?? 0,
      }));
                          ...
                          <tr key={it.id} className="hover:bg-gray-50">
                            ...
                            <td className="px-4 py-3 text-sm text-gray-700">
                              <div className="flex items-center gap-2">
                                ...
                                <div className="text-xs w-10 text-right">{Number(it.percent_complete || 0)}%</div>
                              </div>
