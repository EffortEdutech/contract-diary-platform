...
const WorkProgrammeModal = ({ isOpen, onClose, contractId, authority, isLocked }) => {
  ...
  const loadItems = async (versionNo) => {
    ...
    try {
      ...
      // default: expand all summaries (or anything with children) after load
      // We'll compute after tree memo below; so here just reset for fresh load.
      // Auto-expand Summary nodes (and any parents)
      const parentIds = new Set();
      const childCount = new Map();

      (list || []).forEach((it) => {
        if (it.parent_id) {
          parentIds.add(it.parent_id);
          childCount.set(it.parent_id, (childCount.get(it.parent_id) || 0) + 1);
        }
      });

      const summaryIds = new Set(
        (list || [])
          .filter((it) => it.activity_type === 'Summary')
          .map((it) => it.id)
      );

      const expand = new Set();
      // Expand summaries (good default)
      summaryIds.forEach((id) => expand.add(id));
      // Also expand any node that actually has children
  ...
  const onQuickUpdatePercent = async (item, nextPct) => {
    try {
      await updateProgrammeItem(item.id, {
        ...
        status:
          nextPct >= 100 ? 'Completed' : nextPct > 0 ? 'In Progress' : 'Not Started',
      });
      await loadItems(selectedVersionNo);
    } catch (e) {
  ...
  useEffect(() => {
    ...
    // When filtering, expand everything in filtered tree
  ...
  const visibleRows = useMemo(() => {
    return flattenVisible(filteredTree, expandedIds);
            ...
            <div className="bg-white border rounded-lg overflow-hidden">
              <div className="px-4 py-3 bg-gray-50 border-b">
                ...
                <div className="text-xs text-gray-600 mt-0.5">
                  If versions are empty but activities exist, we still show activities by version
                  number.
                </div>
              </div>
            ...
            <div className="bg-white border rounded-lg overflow-hidden">
              ...
              <div className="px-4 py-3 bg-gray-50 border-b flex items-start justify-between gap-3">
                <div>
                  ...
                  {/* ✅ Search + Filters UI (PASTE THIS WHOLE BLOCK) */}
