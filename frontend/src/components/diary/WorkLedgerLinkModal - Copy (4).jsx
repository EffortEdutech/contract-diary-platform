 ...
 *
 * Scroll fix:
 * - Overlay is scrollable (overflow-y-auto)
 * - Modal has max height (max-h-[92vh])
...
export default function WorkLedgerLinkModal({
  ...
  // Load mapped programmes for currently expanded BOQ
  useEffect(() => {
    const load = async () => {
      if (!isOpen) return;
  ...
  const requestSwitchExpanded = (boqId) => {
    if (expandedBoqId && expandedBoqId !== boqId && isDirty) {
      const ok = window.confirm(
        'You have unsaved changes. Switch BOQ anyway? (Changes remain until Save or Close)'
      );
      if (!ok) return;
    }
  ...
  return (
    // ✅ Overlay scrollable + padding
    <div className="fixed inset-0 z-50 bg-black/50 overflow-y-auto">
      <div className="min-h-full flex items-start justify-center p-4">
        {/* ✅ Modal: max height + flex column */}
